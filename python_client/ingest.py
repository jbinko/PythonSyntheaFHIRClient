...
def main():
	connection_string = "DefaultEndpointsProtocol=https;AccountName=fhirlanding;AccountKey=CmrotBz6cCiWCff468oGsloGEPH8bRlkdpEevz+LjtSSKzYrntSZyRbhicrXUQa9uYgpwxkz3zdRCl7+RrCE9A==;EndpointSuffix=core.windows.net"
	# Create the BlobServiceClient object which will be used to create a container client
	blob_service_client = BlobServiceClient.from_connection_string(connection_string)
        # Create a unique name for the container
	container_name = "fhirsynthea"
	local_output_path = dir_path = os.path.dirname(os.path.realpath(__file__))
	print(local_output_path)
	local_file_path = "/home/vmadmin/synthea/output/fhir/fhir"
	#while(True):
